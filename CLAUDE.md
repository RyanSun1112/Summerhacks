...
- **The dashboard is owner-gated when auth is on**: signed-out visits to dashboard.html client-redirect to `owner.html?next=…` (same-origin `next` only), sign-in returns, sign-out bounces back. join.html and the state/socket APIs stay open — this is front-door UX, not a security boundary. `AUTH_MODE=off` keeps the dashboard open for a no-accounts projector setup.
