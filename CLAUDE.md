...
- **Vibe captures work in local mode too**: Supabase stores them when configured; otherwise rows go in a `captures` table in the shared SQLite db and photos to `data/captures/` (gitignored), same response shape, same `/api/captures/:id/photo` proxy, same live `capture` socket event — clients can't tell the difference. The city map reads venues from public `GET /api/city/venues` (name + approximate coords for every LOCATED venue, unscoped by design) — never from the dashboard's per-account `/venues`, which hides everything but the live venue from signed-out viewers and would leave the city empty.
