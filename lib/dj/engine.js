...
async function selectNextSong(options) {
  ...
  const selectionOverrides = { ...(configOverrides.selection || {}) };
  if (selectionMode != null) selectionOverrides.mode = selectionMode;
  if (guidanceStrength != null) selectionOverrides.guidanceStrength = guidanceStrength;
  const config = mergeConfig({ ...configOverrides, selection: selectionOverrides });
